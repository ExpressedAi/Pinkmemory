...
import { chunkText, ltm_addChunk } from '@/services/memory';
...
const UploaderLTMPage: React.FC = () => {
  const { settings } = useSettings();
  const { clearAllMemories, refreshMemoryCounts, ltmCount } = useMemory();
  ...
  const processChunks = async (chunksToProcess: ProcessedChunk[]) => {
    ...
    if (!settings.apiKeyC) {
      setProcessingStatus("Error: Agent C API Key is not configured in Settings. Cannot process upload.");
      return;
    }
    
    setProcessingStatus(`Processing ${chunksToProcess.length} chunks for LTM...`);
    ...
    for (let j = 0; j < chunksToProcess.length; ++j) {
      setProcessingStatus(`Processing LTM chunk: ${j+1}/${chunksToProcess.length} | ✓ ${ok} | ✗ ${fail}`);
      ...
      try {
        // Use Agent C's key and embedding model
        const embedding = await fetchEmbedding(settings.apiKeyC, chunksToProcess[j].text, settings.modelCEmbed);
        
        // Use Agent C's key but potentially cheaper meta model
        const meta = await fetchMeta(settings.apiKeyC, chunksToProcess[j].text, settings.modelCMeta);
        const metaVector = buildMetaVectorFromAI(meta);
        
        // Save to IndexedDB (LTM)
        await ltm_addChunk({
          text: chunksToProcess[j].text,
          embedding,
          metaVector,
          timestamp: Date.now(),
          agentId: "agent-c-uploader",
          meta,
        ...
        console.error(`Error processing LTM chunk ${j+1} from ${chunksToProcess[j].source}:`, error);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-2 text-gray-800">📄 MetaVector Memory Uploader (to LTM)</h1>
      <p className="text-gray-500 mb-4">
        Upload text or files here. They will be chunked, embedded, meta-scored using Agent C's settings, 
        and saved to the Long-Term Memory store (IndexedDB).
      </p>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 mb-6">
        <div className="mb-1">
          <span className="text-indigo-600 font-medium">Agent C (LTM Uploader):</span> 
          <span className="text-sm text-gray-500 ml-1">{settings.modelCEmbed} / {settings.modelCMeta}</span>
        </div>
        <div className="text-sm text-gray-500">
          API Key Used: {settings.apiKeyC ? settings.apiKeyC.slice(0, 6) + "..." + settings.apiKeyC.slice(-4) : "Not Set"} | 
          Store: IndexedDB (LTM) | Chunks: {ltmCount}
        </div>
      </div>
              ...
              {processedChunks.map((chunk, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4 text-sm">
                    ...
                    {chunk.status === 'success' && <span className="text-green-600">✓ Saved to LTM</span>}
                    {chunk.status === 'error' && (
...
export default UploaderLTMPage;